    ...
    const requestData = async () => {
      try {
        ...
        const req = await axios.get(
          `https://api.lmiforall.org.uk/api/v1/ess/regions/ranksocs/${regionDropdown}`
        );
        ...
        for (const element of data) {
          ...
          const soc_request = await axios.get(
            `https://api.lmiforall.org.uk/api/v1/soc/code/${element.soc}`
          );
    ...
    <>
      <div className={styles.container}>
        ...
        <div className={styles.dropdown}>
          <select
            id="Region-Filter"
            onChange={(e) => setRegionDropdown(e.target.value)}
          >
        ...
        <p className={styles.subheading}>
          Top {!loading ? barChartData.labels.length : 0} hardest jobs to fill
          in chosen area
        </p>
